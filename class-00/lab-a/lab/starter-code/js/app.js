...
function displayPics(){
  ...
  // the rando const is local to that function and is not defined outsidew of it
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      // (event.target.id + ' has ' + allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views');
    }
  }
...
function showList() {
  for(let i = 0; i < allProducts.length; i++) {
    ...
    // allProducts[i].name + ' has ' + allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views';
